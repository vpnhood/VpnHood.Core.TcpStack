using System.Net;
using VpnHood.Core.TcpStack.Abstractions;

namespace VpnHood.Core.TcpStack.LwIP;

/// <summary>
/// Represents an accepted TCP connection from the lwIP TCP stack.
/// Similar to <see cref="System.Net.Sockets.TcpClient"/>, it holds endpoints and owns the stream.
/// </summary>
public sealed class LwipTcpClient(LwipTcpStream stream, IPEndPoint localEndPoint, IPEndPoint remoteEndPoint)
    : ITcpClient
{
    public IPEndPoint LocalEndPoint { get; } = localEndPoint;
    public IPEndPoint RemoteEndPoint { get; } = remoteEndPoint;
    public Stream Stream => stream;

    public void Dispose() => stream.Dispose();
    public ValueTask DisposeAsync() => stream.DisposeAsync();
}
